...
public class GUIControls : MonoBehaviour{
	...
	public Transform ParenHandlers;
	void Start(){
					...
					if(blockGui){
						...
						if(rpgCharacterController.weapon != Weapon.TWOHANDAXE){
							if(GUI.Button(new Rect(1115, 410, 100, 30), "2 Hand Axe")){
								FirstWeapon.transform.parent = ParenHandlers;
								FirstWeapon.GetComponent<Rigidbody>().useGravity = false;
								FirstWeapon.GetComponent<Rigidbody>().isKinematic = true;
								FirstWeapon.GetComponent<Collider>().enabled = false;
								FirstWeapon.transform.position = ParenHandlers.position;
								FirstWeapon.transform.rotation = ParenHandlers.rotation;
